...
void W25QXX_Init(void)
{
  ...
  RCC_AHB1PeriphClockCmd(W25QXX_CS_RCC, ENABLE );               //PORT时钟使能
  GPIO_InitStructure.GPIO_Pin = W25QXX_CS_PIN;                  //PIN
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;                 //输出功能
  GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;                //推挽输出
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;            //100MHz
  GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;                  //上拉
  GPIO_Init(W25QXX_CS_GPIO, &GPIO_InitStructure);               //初始化
  W25QXX_CS_H;				                                    //SPI FLASH不选中
  
  SPI3_Init();		   	                                        //初始化SPI
  W25qxxSPI_SetSpeed(SPI_BaudRatePrescaler_2);                  //设置为18M时钟,高速模式
...
{
  u8 byte=0;
  W25qxx_CsEnable();                                                  //使能器件
  W25qxxSPI_ReadWriteByte(W25X_ReadStatusReg);                       //发送读取状态寄存器命令
  byte=W25qxxSPI_ReadWriteByte(0Xff);                                //读取一个字节
  W25qxx_CsDisable();                                                  //取消片选
  return byte;
}
/*********************************************************************************************
* 名称:W25QXX_Write_SR
...
{
  W25qxx_CsEnable();                                                  //使能器件
  W25qxxSPI_ReadWriteByte(W25X_WriteStatusReg);                      //发送写取状态寄存器命令
  W25qxxSPI_ReadWriteByte(sr);               	                //写入一个字节
  W25qxx_CsDisable();                                                  //取消片选
}
/*********************************************************************************************
...
{
  W25qxx_CsEnable();                          	                //使能器件
  W25qxxSPI_ReadWriteByte(W25X_WriteEnable); 	                //发送写使能
  W25qxx_CsDisable();                           	                //取消片选
}
/*********************************************************************************************
...
{
  W25qxx_CsEnable();                                                  //使能器件
  W25qxxSPI_ReadWriteByte(W25X_WriteDisable);                        //发送写禁止指令
  W25qxx_CsDisable();                                                  //取消片选
}
//读取芯片ID
//返回值如下:
...
u16 W25QXX_ReadID(void)
{
  u16 Temp = 0;
  W25qxx_CsEnable();
  W25qxxSPI_ReadWriteByte(0x90);                                     //发送读取ID命令
...
void W25QXX_Read(u8* pBuffer,u32 ReadAddr,u16 NumByteToRead)
{
  u16 i;
  W25qxx_CsEnable();                            	                //使能器件
  W25qxxSPI_ReadWriteByte(W25X_ReadData);         	                //发送读取命令
  W25qxxSPI_ReadWriteByte((u8)((ReadAddr)>>16));  	                //发送24bit地址
  W25qxxSPI_ReadWriteByte((u8)((ReadAddr)>>8));
  W25qxxSPI_ReadWriteByte((u8)ReadAddr);
  for(i=0;i<NumByteToRead;i++)
  {
    pBuffer[i]=W25qxxSPI_ReadWriteByte(0XFF);   	                //循环读数
  }
  W25qxx_CsDisable();
}
...
void W25QXX_Write_Page(u8* pBuffer,u32 WriteAddr,u16 NumByteToWrite)
{
  ...
  W25qxx_CsEnable();                            	                //使能器件
  W25qxxSPI_ReadWriteByte(W25X_PageProgram);      	                //发送写页命令
  W25qxxSPI_ReadWriteByte((u8)((WriteAddr)>>16)); 	                //发送24bit地址
  W25qxxSPI_ReadWriteByte((u8)((WriteAddr)>>8));
  W25qxxSPI_ReadWriteByte((u8)WriteAddr);
  for(i=0;i<NumByteToWrite;i++)W25qxxSPI_ReadWriteByte(pBuffer[i]);  //循环写数
  W25qxx_CsDisable();                            	                //取消片选
...
void W25QXX_Erase_Chip(void)
{
  ...
  W25QXX_Wait_Busy();
  W25qxx_CsEnable();                            	                //使能器件
  W25qxxSPI_ReadWriteByte(W25X_ChipErase);        	                //发送片擦除命令
  W25qxx_CsDisable();                            	                //取消片选
...
void W25QXX_Erase_Sector(u32 Dst_Addr)
{
  ...
  W25QXX_Wait_Busy();
  W25qxx_CsEnable();                            	                //使能器件
  W25qxxSPI_ReadWriteByte(W25X_SectorErase);      	                //发送扇区擦除指令
  W25qxxSPI_ReadWriteByte((u8)((Dst_Addr)>>16));  	                //发送24bit地址
...
{
  W25qxx_CsEnable();                           	 	        //使能器件
  W25qxxSPI_ReadWriteByte(W25X_PowerDown);                           //发送掉电命令
  W25qxx_CsDisable();                            	                //取消片选
}
