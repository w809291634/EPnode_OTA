...
//	Encryption();
  ...
  RCC_AHB1PeriphClockCmd(TERM_GPIO_CLK, DISABLE);
  //重置运行灯、按键引脚
  LED_OFF(); 
  GPIO_DeInit(LED_RUN_PORT);
  GPIO_DeInit(REC_DETECT_PORT);
  RCC_AHB1PeriphClockCmd(LED_RUN_CLK, DISABLE);
  RCC_AHB1PeriphClockCmd(REC_DETECT_CLK, DISABLE);
...
char getcc(void){
	...
	uint8_t result=0;
	unsigned counter=0;
//	unsigned bootdelay = OVER_TIME*4000000;
...
//	for(counter=0;counter<(bootdelay);counter++){
//		if(ringbuf_read(&result)==1) break;
...
//		}
//	}
//	if(counter==(bootdelay)){
//		printk("\t等待用户输入命令超时，进入用户程序\n");
////		IWDG_ReloadCounter();  
...
//                  /* erase user application area */
                    ...
                    /* Define the number of page to be erased */
										{
											size = stringtoul((const char*)file_size,&ret);
											uint32_t startsec = FLASH_IF_GetSector(APPLICATION_ADDRESS);
											uint32_t stopsec = FLASH_IF_GetSector(APPLICATION_ADDRESS+size);
											/* Erase the FLASH pages */
											FLASH_If_Erase(startsec,stopsec);
										}
                    Send_Byte(YMODEM_ACK);
