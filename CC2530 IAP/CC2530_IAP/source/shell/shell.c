...
union uncmd {
	struct {// 命令号分为以下五个部分  
		unsigned long CRC2      : 8;
		unsigned long CRC1      : 8;///< 低十六位为两个 crc 校验码
		unsigned long Sum       : 5;///< 命令字符的总和
		unsigned long Len       : 5;///< 命令字符的长度，5 bit ，即命令长度不能超过31个字符
		unsigned long FirstChar : 6;///< 命令字符的第一个字符
	}part;
	unsigned long ID;               ///< 由此合并为 32 位的命令码
};
...
static void   shell_tab         (struct shell_input * shellin) ;
	...
	static void   shell_show_history(struct shell_input * shellin,long LastOrNext);
#else
...
*/
static struct shellcommand *shell_search_cmd(cmd_root_t * root , unsigned long cmdindex)
{
...
/**
  ...
  * @return   成功返回 0
*/
static long shell_insert_cmd(cmd_root_t * root , struct shellcommand * newcmd)
{
	struct shellcommand * command ;
	cmd_entry_t **node = &ROOT(root) ;
...
/** 
  ...
  * @return 返回检出命令的条目数
*/
static long checkout(char * str,long len,struct shellcommand ** checkout , long checkmax)
{
	...
	index = ((unsigned long)(*str)<<26) | (len << 21) ;
	end = (unsigned long)(*str + 1)<<26 ; 
...
static void shell_parse(cmd_root_t * cmdroot , struct shell_input * shellin)
{
	...
	unsigned long len = 0;
	unsigned long sum = 0;
	unsigned long fcrc8 = 0;
	unsigned long bcrc8 = 0;
	char  *  str = shellin->cmdline;
