  ...
  if(rf_Link_On == 1) {
    ...
    if(err == RT_EOK) {
      if(mb_recv == 1) {  //邮件来源：通讯串口中断
        ...
        if(length <= 0) return;
        if(strstr(recv, "+MIPCLOSE") != NULL) {  //接收到TCP连接断开的消息
          rf_Link_On = 0;
        } else {
          at_recv(recv);
#ifndef CAT1_SERIAL      /*终端节点*/
...
int16_t rf_if_send(uint8_t *pdata, uint16_t length, uint16_t timeout_ms) {
  ...
  //发送数据到服务器
  if(rf_if_dev == RF_IF_CAT1) {        //CAT1透传发送：连接状态为已连接
...
void rf_if_Link(void) {
  ...
  //定时发送心跳包，检测是否出现断线
  while(rf_Link_On == 1) {
    if(rf_if_heart_try++ > 6) {    //心跳包发送6次无回复：连接已断开，断线重连
      rf_if_heart_try = 0;
      rf_Link_On = 0;
      rf_if_NetStatus(RESET);
      break;
